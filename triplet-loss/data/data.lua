...
if paths.filep('dataset.t7') then
   ...
   local datasetPaths = {}
   datasetPaths.base = '/home/wolf1/oriterne/DATA/real_data_RGB'
   ...
   for _, t in ipairs {'train', 'test'} do
      ...
      local letters=letters --get from help_funcs
      print(letters)

      local dataSize = tonumber(sys.execute('find ' .. datasetPaths[t] .. ' -iname "*.png"| wc -l'))
	print(#identities)
	print(dataSize)

      dataset[t] = {
         data = torch.Tensor(dataSize, 3, imageSide, imageSide),
         label = torch.IntTensor(dataSize),
	 letter = torch.IntTensor(dataSize),
         index = torch.zeros(#identities,#letters, 2),
      }
...
--         dataset[t].index[id][1] = count + 1
	 ...
	 local prev_let=nil
	    ...
	    current_let= reverse_letters[img:split('[.(]')[1]]
		--TODO ugly code
	    if current_let~=prev_let then 
		dataset[t].index[id][current_let][1]=count+1
		if first_iter then	
			first_iter=false
		else
			dataset[t].index[id][prev_let][2]=count
		end
	    end
	    ...
	    dataset[t].letter[count] =	current_let
	    prev_let=current_let
         end
         dataset[t].index[id][prev_let][2] = count
         collectgarbage()
      end
...
local getBatch = function(nb, t, mode, epoch)
   ...
   local letters = torch.Tensor(batchSize)
   ...
   for i = 1, batchSize do
      ...
      letters[i] = dataset[t].letter[loc]
      if mode ~= 'soft-neg2' then
         pImgs[i]  = dataset[t].data [math.random(
            dataset[t].index[labels[i]][letters[i]][1],
            dataset[t].index[labels[i]][letters[i]][2]
         )]
      end
   end
   ...
   if mode == 'hard-neg' then
      for i = 1, batchSize do
         ...
         norms[letters:ne(letters[i])] = norms:max()  --my addition
	 --print(norms:ne(norms:max()):sum())
   ...
   elseif mode == 'soft-neg2' then
      for i = 1, batchSize do
         ...
         norms[labels:eq(labels[i])] = norms:max()+2  --delete the 1 if... with it it makes the 
         norms[letters:ne(letters[i])] = norms:max()+1  --my addition
