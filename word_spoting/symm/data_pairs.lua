...
local paths1={}
local word_spoting_folders={}
local dataSize={}
local paths1_flatened={}


local flaten=function(table1)
	print('flaten')
	local result={}
	for k,folder in pairs(table1) do
--		print(k)
		for _,file_path in ipairs(folder) do
--			print(file_path)
...
--    else	
	...
	for _, t in ipairs {'train', 'test'} do
		...
		if paths.filep(pre_saved_path) then
			paths1[t]=torch.load(pre_saved_path)  
		else
			print('prepering from '..datasetPaths[t])
			paths1[t]=preper_data_word_spoting(datasetPaths[t])
			torch.save(pre_saved_path,paths1[t])
		end
		word_spoting_folders[t]=set_word_spoting_folders(paths1[t])
		paths1_flatened[t]=flaten(paths1[t])

	      		 	--start	
		dataSize[t] = tonumber(sys.execute('find ' .. datasetPaths[t] .. ' -iname "*.jpg"| wc -l'))
		print(dataSize[t])
		print(#paths1_flatened[t])
	      	dataSize[t]=math.min(limit_datasize[t], dataSize[t])
--[[
	      	dataset[t] = {
...
local toNormalize
local my_normalize_pairs = function(t,kind)
	print('normalizing data for PAIRS. kind: '..kind..' t:'..t..' following mean and std if given')
	print(mean_)
	print(std_)
	if kind=='perImage' then
		print('setting toNormalize=ture')
		toNormalize=true
	else 
...
local shuffleShuffle = function(t)
   shuffle = torch.randperm(dataSize[t])
end
...
local my_getNbOfBatches = function(bS)
   batchSize = bS
   for _, t in ipairs {'train', 'test'} do
      nbOfBatches[t] = math.floor(dataSize[t] / batchSize)
   end
...
local my_getBatch = function(nb, t )
   ...
   for i = 1, batchSize do
   	local loc = shuffle[i + offset] -- original location in data set
        local p1= paths1_flatened[t][loc]   
	local p2
--	print(p1)      
	local label = paths.basename(paths.dirname(p1))
	local label2
        local same=math.random(1000)>500
      
        if same then
  	    
	    if #paths1[t][label]==1 then --single word  --choose another word that is not single
         	while #paths1[t][label]==1 do
                        loc = math.random(dataSize[t]) -- choose another word randomly from whole dataset
  		        p1= paths1_flatened[t][loc]                     
			label = paths.basename(paths.dirname(p1))
	        end
	    end
	    label2=label
	    p2=paths1[t][label][math.random(#paths1[t][label])]
	    while  p1==p2 do
	 	p2=paths1[t][label][math.random(#paths1[t][label])]
   	    end	 
--	    print('same'..p2)
        else
            label2=word_spoting_folders[t][math.random(#word_spoting_folders[t])]
		--TODO while label==label2
	    while label2==label do
		label2=word_spoting_folders[t][math.random(#word_spoting_folders[t])]	
	    end
	    p2=paths1[t][label2][math.random(#paths1[t][label2])]
--	    print('not same'..p2)
        end
      
        inputs[i][1]  = image.scale(image.load(p1),imageSide,imageSide)
	if toNormalize then
 	     local mean=inputs[i][1]:mean()
	     local std=inputs[i][1]:std()
 	     inputs[i][1]:add(-mean);
      	     inputs[i][1]:mul(1.0/std);
  	end
	inputs[i][2]  = image.scale(image.load(p2),imageSide,imageSide)
	if toNormalize then
 	     local mean=inputs[i][2]:mean()
	     local std=inputs[i][2]:std()
 	     inputs[i][2]:add(-mean);
      	     inputs[i][2]:mul(1.0/std);
  	end
	edit_dists[i]=string.levenshtein_4_files(label,label2)	
   end
...
function display_images(inputs,labels)
	labels=labels:int()
	...
	print(labels2)
        s={}
        for i=1,50 do
                 table.insert(s,image.toDisplayTensor(inputs[i]))
        end
